...
SELECT employee_id, first_name ,title, last_name,levels
FROM employee
...
select city , sum(total) as Total from  customer inner join invoice 
on customer.customer_id=invoice.customer_id    group by  city order by total desc   limit 1;
